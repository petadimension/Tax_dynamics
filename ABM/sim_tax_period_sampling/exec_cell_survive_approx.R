...
Tmax <- 21.0 # 4.0 # (hours) end of experiment
min_step <- 1.0
...
is_expdata <- "no"
if( is_expdata=="yes" ) {
	t_periods <- as.numeric( read.table(file="../data/t_period.txt",header=T)$t_period ) ## exp
} else {
	t_periods <- read.table("optim_t_period.txt",header=F)$V1 ## sim
}

t_intervals <- read.table("optim_t_interval.txt",header=F)$V1
...
## output ##
sfn <- paste("TaxOnOffApo_num",Ninit,"seed",seedn,".txt",sep="")
